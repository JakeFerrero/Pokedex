...
const effectivenessMatrix: number[][] = [
...
export function calculateTypeWeaknesses(types: Type[]): TypeEffectivenessMap {
  ...
  for (const type of types) {
    const defIndex = typeIndices.indexOf(type);
    for (let i = 0; i < effectivenessMatrix.length; i++) {
      effectivenesses[i] *= effectivenessMatrix[i][defIndex];
    }
  }
